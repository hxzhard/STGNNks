...
def get_data(lambda_I=0, Adjacent_threshold=200):
    data_path = '/data/hxz/ccst/CCST-main/generated_data/'
    with open(data_path + 'Adjacent_' + str(Adjacent_threshold), 'rb') as fp:
        print(fp)
        adj_0 = pickle.load(fp)
    X_data = np.load(data_path + 'features_array_after_removal_low_var.npy')
...
def draw_map(args, lambda_I, cell_batch_info, adj_0):
    ...
    import openpyxl
    workbook = openpyxl.load_workbook('/data/hxz/ccst/CCST-main/merfish/ccst_ksum_zlz/V1_Mouse_Kidney_result.xlsx')
    start_cell_index = 0
    import matplotlib.cm as cm
    colors = cm.viridis(np.linspace(0, 1, n_clusters))
    all_cluster = plt.scatter(x=np.arange(n_clusters), y=np.arange(n_clusters), s=100, c=np.arange(n_clusters))
    plt.clf()
    for sheet_name in ['Batch 1']:
        worksheet = workbook[sheet_name]
        sheet_X = [item.value for item in list(worksheet.columns)[1]]
        sheet_X = sheet_X[1:]
        sheet_X[sheet_X == ''] = 0.0
        ...
        fig_name = 'lambdaI' + str(lambda_I) + '/cluster_' + sheet_name.replace(' ', '')
        plt.title(sheet_name)
        sc_cluster = plt.scatter(x=sheet_X, y=sheet_Y, s=10,
                                 c=colors[cell_cluster_type_list[start_cell_index: end_cell_index]])
        cb_cluster = plt.colorbar(all_cluster, boundaries=np.arange(n_clusters + 1) - 0.5).set_ticks(
    ...
    np.savetxt(args.result_path + 'lambdaI' + str(lambda_I) + '/cluster_' + str(n_clusters) + '_barplot.txt', barplot,
               fmt='%3d', delimiter='\t')
    norm_barplot = barplot / (source_cluster_type_count.reshape(-1, 1))
    np.savetxt(args.result_path + 'lambdaI' + str(lambda_I) + '/cluster_' + str(n_clusters) + '_barplot_normalize.txt',
               norm_barplot, fmt='%3f', delimiter='\t')
...
def main(args):
    ...
    adj_0, adj, X_data = get_data(lambda_I)
    ...
    if args.cluster:
        ...
        dav = metrics.davies_bouldin_score(X_embedding, cluster_labels)
        cal = metrics.calinski_harabasz_score(X_embedding, cluster_labels)
        sil = metrics.silhouette_score(X_embedding, cluster_labels)
        sdbw = np.round(S_Dbw(X_embedding, cluster_labels), 5)
        print("dav:", dav)
        print("cal:", cal)
        print("sil:", sil)
        print("sdbw:", sdbw)
        adata = sc.read_visium("/data/hxz/10xdata/V1_Mouse_Kidney (Coronal)")
        ...
        plt.savefig(f"/data/hxz/ccst/CCST-main/picture/V1_Mouse_Kidney_clusters_Umap.jpg")
        ...
        plt.savefig(f'//data/hxz/ccst/CCST-main/picture/V1_Mouse_Kidney_clusters.jpg', bbox_inches='tight',
                    dpi=150)
        ...
        np.savetxt(args.result_path + 'lambdaI' + str(lambda_I) + '/types.txt', np.array(all_data), fmt='%3d',
                   delimiter='\t')
    ...
    if args.diff_gene:
        ...
        for i in range(n_clusters):  # num_cell_types
            ...
            for ind, diff_gene_index in enumerate(min_p_index):
                ...
                if (flag_0 and flag_1) or (min_p[ind] > 0.05):
                    break
                if cells_features_cur_type[:, diff_gene_index].mean() > cells_features_other_type[:,
                                                                        diff_gene_index].mean():
                    if not flag_0:
                        gene_cur_type.append(gene[diff_gene_index])  # diff gene that higher in subpopulation 0
                        diff_gene_sub.append('cur ' + str(len(gene_cur_type) - 1))
                    else:
            ...
            file_handle = open(args.result_path + 'lambdaI' + str(lambda_I) + '/cluster' + str(i) + '_diff_gene.txt',
                               mode='w')
            for m in range(len(diff_gene_sub)):
                file_handle.write(min_p_gene[m] + '\t' + str(min_p[m]) + '\t' + diff_gene_sub[m] + '\n')
            file_handle.close()
            file_handle_0 = open(args.result_path + 'lambdaI' + str(lambda_I) + '/cluster' + str(i) + '_gene_cur.txt',
                                 mode='w')
            for m in range(len(gene_cur_type)):
                file_handle_0.write(gene_cur_type[m] + '\n')
            file_handle_0.close()
            file_handle_1 = open(args.result_path + 'lambdaI' + str(lambda_I) + '/cluster' + str(i) + '_gene_other.txt',
                                 mode='w')
            for m in range(len(gene_other_type)):
                file_handle_1.write(gene_other_type[m] + '\n')
            file_handle_1.close()
