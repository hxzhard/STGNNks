...
def Kmeans_cluster(X_embedding, n_clusters, merge=False):
    ...
    sil = metrics.silhouette_score(X_embedding, cluster_labels, metric='euclidean')
    ...
    # Ann_df = pd.read_csv('/data/hxz/ccst/CCST-main/dataset/V1_Breast_Cancer_Block_A_Section_1//metadata.tsv', sep='\t')
    # ARI = np.round(metrics.adjusted_rand_score(cluster_labels, Ann_df['fine_annot_type']), 3)
    # NMI = np.round(metrics.normalized_mutual_info_score(cluster_labels, Ann_df['fine_annot_type']), 3)

    return cluster_labels, sil,dav,cal,sdbw#,ARI,NMI
